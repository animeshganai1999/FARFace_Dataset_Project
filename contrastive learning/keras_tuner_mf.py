...
def build_model(hp):
    ...
    # Define the loss weights as tunable hyperparameters
    concatenate_weight = hp.Float('concatenate_weight', min_value=0.0, max_value=1.0, default=0.5)
    model_weight = hp.Float('model_weight', min_value=0.0, max_value=1.0, default=0.5)
    ...
    # Compile the model with tunable loss weights and learning rate
    model.compile(
        optimizer=Adam(learning_rate=hp.Choice('learning_rate', values=[1e-4, 5e-5, 1e-5,1e-6])),
        loss=losses,
        loss_weights={'concatenate': concatenate_weight, 'model': model_weight},
...
tuner = keras_tuner.RandomSearch(
    ...
    directory='tuner_dir',  # Specify a directory to store the tuner results
    project_name='gender_prediction_tuner'  # Specify a project name for the tuner
)
...
# Perform the hyperparameter search using the fit method with custom generator
tuner.search(
    custom_generator(x_train, y_gender_train, anchor_indices, positive_indices, negative_indices, batch_size),
    epochs=10,
...
best_concat_weight = best_hps.get("concatenate_weight")
best_model_weight = best_hps.get("model_weight")
print("Best values")
print(best_lr,best_concat_weight, best_model_weight)
...
final_model.save('vgg_face_triplet_loss_full_mf_hyperparameter_'+str(epochs)+'_epochs.h5')
