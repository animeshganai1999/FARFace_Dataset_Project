...
image_path = "./../All Face Images with And Without Mask (N95)/Australia_resized/normal/Alana King_0.jpg"
...
img_face_only = img_face_only.astype(np.float32)  # Convert to 32-bit floating-point
...
#     for j in range(width):
#         if (
#             (img_HSV.item(i, j, 0) <= 170)
#             and (140 <= img_YCrCb.item(i, j, 1) <= 170)
#             and (90 <= img_YCrCb.item(i, j, 2) <= 120)
#         ):
#             blue.append(img_face_only[i, j].item(0))
#             green.append(img_face_only[i, j].item(1))
#             red.append(img_face_only[i, j].item(2))
...
# if blue and green and red:  # Check if skin pixels are present
#     skin_tone_estimate_BGR = [np.mean(blue), np.mean(green), np.mean(red)]
#     print("mean skin tone estimate (BGR):", skin_tone_estimate_BGR)
# else:
